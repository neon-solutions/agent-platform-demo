...
import { z } from "zod";
import { getAppSandbox, runInSandbox, startDevServer } from "../../../lib/sandbox";
import { snapshotTenantBranch, restoreTenantSnapshot, type Plan } from "../../../lib/neon";
import {
  insertCheckpoint,
...
export function buildTools(proto: PrototypeRow) {
  ...
  return {
    ...
    readFile: createTool({
      id: "readFile",
      description: "Read the full contents of a file in the app (path relative to the project root).",
    ...
    restoreCheckpoint: createTool({
      ...
      execute: async ({ checkpointId }) => {
        ...
        const sb = await sandbox();
        if (cp.gitSha) {
          await runInSandbox(sb, `git checkout -f ${cp.gitSha} -- . && git checkout ${cp.gitSha} 2>/dev/null || git reset --hard ${cp.gitSha}`);
          await startDevServer(sb, databaseUrl);
        }
