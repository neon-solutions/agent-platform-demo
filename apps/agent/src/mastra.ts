...
- Database: use **Drizzle**. Define tables in \`lib/schema.ts\`. For the schema to exist at runtime, keep \`ensureSchema()\` in \`lib/db.ts\` in sync (idempotent \`CREATE TABLE IF NOT EXISTS …\` / \`ALTER TABLE … ADD COLUMN IF NOT EXISTS …\`), or run migrations via \`runCommand\`. Preserve the starter's Neon HTTP-compatible \`db.execute(sql.raw("…"))\` pattern; do not replace it with a tagged \`sql\` template. Read from Server Components and write from Server Actions.
